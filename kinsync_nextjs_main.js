...
const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const savedUser = localStorage.getItem('kinsync_user');
      if (savedUser) {
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        try {
          ...
          if (typeof window !== 'undefined') {
            localStorage.setItem('kinsync_user', JSON.stringify(userData));
          }
          setUser(userData);
          setIsAuthenticated(true);
          setLoading(false);
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        try {
          ...
          if (typeof window !== 'undefined') {
            localStorage.setItem('kinsync_user', JSON.stringify(newUser));
          }
          setUser(newUser);
          setIsAuthenticated(true);
          setLoading(false);
  ...
  const logout = () => {
    if (typeof window !== 'undefined') {
      localStorage.removeItem('kinsync_user');
    }
    setUser(null);
    ...
    <div className="w-full max-w-md mx-auto">
      <div className="text-center mb-8">
        <div className="flex items-center justify-center gap-2 mb-4">
          ...
          <span className="text-2xl font-bold text-gray-800">Kinsync</span>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xl font-bold text-gray-800">Kinsync</span>
            </div>
